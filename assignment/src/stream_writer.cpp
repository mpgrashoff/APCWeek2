...
namespace writers {
    ...
    stream_writer::stream_writer(std::string_view fname) : m_out{std::make_unique<std::ofstream>(fname.begin())} {
    }

    stream_writer::stream_writer(std::unique_ptr<std::ostream> out) noexcept : m_out{std::move(out)} {
